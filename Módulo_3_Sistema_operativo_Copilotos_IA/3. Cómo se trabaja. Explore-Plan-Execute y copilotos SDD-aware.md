# 📄 Cómo se trabaja: Explore-Plan-Execute y copilotos SDD-aware 🔴 — 22 min
...
Explore es barato. Haiku leyendo 30 archivos cuesta menos de un café. Tu tiempo es más caro.

Mirar Explore te ata a leer 30 archivos. El sentido de Explore es que sintetiza por ti. Si lo supervisas turn-by-turn, has perdido el ahorro de contexto.
...
Antídoto: si una invariante está en hooks, no la repitas en CLAUDE.md ni en el prompt. Y al revés.
...
Antídoto: nuevas tareas → nueva sesión. La inversión de re-cargar contexto vía CLAUDE.md y skills es mucho menor que el coste de arrastrar contexto contaminado.
...
> 💡 Heurística: si llevas 10+ turnos y sientes que estás "luchando con el agente", es probable que el contexto esté contaminado. /clear, recarga CLAUDE.md (auto), reformula el prompt incorporando lo aprendido.

## Táctica 5 · Skills sobre instructions one-shot

Las skills tienen una propiedad sutil que CLAUDE.md no: persisten en auto-compact. Cuando el agente compacta, las skills invocadas se vuelven a inyectar (los primeros 5K tokens de cada una, dentro de un budget compartido de 25K).
...
¿Excluye datos sensibles? Sí (excluye password_hash, secrets).
