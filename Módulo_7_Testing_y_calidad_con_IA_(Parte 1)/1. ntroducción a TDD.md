...
# Ciclo de Test-Driven Development (TDD)

## Objetivo

Implementar una funcionalidad siguiendo el ciclo clásico de **Test-Driven Development (TDD)**:

1. Escribir un test.
2. Verificar que falle.
3. Escribir el código mínimo para hacerlo pasar.
...
En esta fase se escribe un test para una funcionalidad que todavía no existe.

Acciones mostradas en el diagrama:
...
Es decir, el test debe fallar porque el comportamiento aún no está implementado.

↓

### Paso 3. GREEN

Una vez el test falla, se escribe el código mínimo necesario para hacerlo pasar.
...
En esta fase no se busca escribir la mejor solución, únicamente una implementación suficiente para que el test pase.

↓

### Paso 4. REFACTOR

Cuando todos los tests pasan, se mejora el código.
...
El objetivo es mejorar el diseño sin cambiar el comportamiento observable.

También aparece como resultado:
...
Después del refactor se vuelve nuevamente a la fase **RED** para implementar la siguiente funcionalidad.
...
| **REFACTOR** | Mejorar el código existente  | Clean Code         |

---

## Lo que muestra el diagrama

El diagrama muestra explícitamente el ciclo clásico de **Test-Driven Development (TDD)**:

1. **RED** → escribir un test que falle.
2. **GREEN** → escribir el código mínimo para que el test pase.
3. **REFACTOR** → mejorar el código manteniendo los tests en verde.
4. Repetir el ciclo para la siguiente funcionalidad.
...
| Fase           | TDD clásico (humano)             | TDD con agentes de IA (2026)                                    |
| -------------- | -------------------------------- | --------------------------------------------------------------- |
| Lista de tests | Notas en papel o pizarra         | Documento markdown (`spec.md`, `CLAUDE.md`) co-creado con IA    |
| Red            | Humano escribe un test que falla | Frecuentemente se combina con Green en un solo prompt           |
| Green          | Humano escribe el código mínimo  | El agente genera el código; humano revisa                       |
| Refactor       | Humano refactoriza               | **Fase más descuidada** — los agentes la saltan si no se fuerza |
| Granularidad   | Pasos muy pequeños               | Los incrementos son más grandes                                 |
| Propósito      | Diseño + confianza               | Todo lo anterior + **mantener al humano en el bucle**           |
...
Incluye instrucciones TDD que Claude lee en cada sesión. Ejemplo mínimo:
...
Permiten crear subagentes especializados (tdd-test-writer, tdd-implementer, tdd-refactorer), cada uno con su contexto aislado.

### Hooks (PostToolUse)

Ejecutan los tests automáticamente tras cada edición de archivo.

### TDD Guard (github.com/nizos/tdd-guard)

Herramienta open source que bloquea al agente si intenta saltarse tests o sobre-implementar. Enforcement automatizado del ciclo.
...
Archivos que se inyectan como contexto persistente. La guía oficial de Cursor recomienda: "Sé explícito sobre que estás haciendo TDD. Dile al agente que ejecute los tests y confirme que fallan. Di explícitamente que no escriba código de implementación en esta etapa."

### Composer Agent Mode

Modo agéntico multi-archivo para el ciclo completo.
...
| --------------------------- | ------------------------------------------------------------------------------------------------------ | --------------------------------------------------- |
| **Vibe coding**             | Te entregas al output de la IA; no te importa el código                                                | Prototipos, scripts desechables, exploración rápida |
| **Augmented coding (Beck)** | Te importan el código, la complejidad, los tests y la cobertura. Usas IA sin renunciar a la ingeniería | Código de producción, equipos profesionales         |
| **Spec-Driven Development** | La especificación es el artefacto primario; la IA implementa                                           | Sistemas complejos, alto grado de colaboración      |
