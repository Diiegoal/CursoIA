...
* Lost in the middle: la atención se sesga hacia los extremos del contexto.

* Attention dilution: la complejidad cuadrática del attention significa que 100k tokens producen 10.000M relaciones por par. Más tokens = más ruido a filtrar.

* Distractor interference: contenido similar pero irrelevante (otro fragmento de código que se parece pero no es) confunde al modelo más que el ruido aleatorio.
...
| Tipo                      | Qué es                                                         | Ejemplo                                                                      |
| ------------------------- | -------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| Código relevante          | Los archivos que el modelo necesita LEER y los que va a EDITAR | El servicio + el endpoint + el test del feature                              |
| Convenciones del proyecto | Estándares, estilo, decisiones arquitectónicas                 | "Usamos repository pattern, no inyectamos DB en endpoints"                   |
| Estado actual             | Qué se acaba de cambiar, qué está roto                         | "El test X está failing porque..."                                           |
| Intent / spec             | Qué quieres lograr y por qué                                   | "Añadir auth con Google OAuth manteniendo compatibilidad con auth existente" |
| Restricciones             | Qué NO hacer                                                   | "No introduzcas dependencias nuevas sin preguntar"                           |
| Memoria persistente       | Conocimiento que se mantiene entre sesiones                    | El archivo `AGENTS.md` / `CLAUDE.md`                                         |
| Documentación externa     | Docs de librerías, RFCs, papers                                | URL de las docs de FastAPI 0.115                                             |
| Histórico de la sesión    | Lo que ya hablaste con el agente en este turn                  | Los últimos N mensajes                                                       |
...
| -------------------------------------- | ----------------------- | ------------------------------------------------------------ |
| `AGENTS.md`                            | OpenAI/Codex (ago 2025) | Estándar de facto multi-vendor                               |
| `CLAUDE.md`                            | Anthropic               | Anthropic-only nativo, pero muchos proyectos mantienen ambos |
| `.cursorrules` / `.cursor/rules/*.mdc` | Cursor                  | Cursor-only nativo                                           |
| `.clinerules`                          | Cline                   | Cline-only nativo                                            |
| `.github/copilot-instructions.md`      | GitHub Copilot          | Copilot only                                                 |
...
* Mínimo y de alta señal: si pones obviedades, el modelo las ignora junto con lo importante. Empieza por una sección "Gotchas" — qué desviaciones del comportamiento por defecto debe conocer.

* Comandos clave, no toda la documentación. El modelo puede leer la doc al ejecutar just --list.

* Convenciones explícitas: "X SÍ, Y NO". Las reglas en negativo funcionan tan bien como las positivas.

* Versionado: stale instructions son peor que no-instructions. Si actualizas la versión de FastAPI, actualiza el archivo.

* Hooks deterministas, no instrucciones: si quieres que NUNCA se añada Co-Authored-By: Claude, ponlo en settings.json como hook, no en AGENTS.md como deseo. Lo que se puede automatizar no debe estar en lenguaje natural.
...
| ---------------------------- | ----------- | -------------------------- |
| Claude Opus 4.7              | 1M tokens   | GA desde 16 abril 2026     |
| Claude Sonnet 4.6 / Opus 4.6 | 1M tokens   | —                          |
| Claude Haiku 4.5             | 200K tokens | —                          |
| GPT-5.4                      | 1M tokens   | Pricing escalonado a >272K |
| GPT-5.3-Codex                | 1M tokens   | Codex-specialized          |
| Gemini 3.1 Pro               | 2M tokens   | Pricing escalonado >200K   |
...
* Crea un AGENTS.md mínimo y de alta señal en cada repo. Mantenlo bajo 200 líneas. Refresca cuando cambien convenciones o versiones.

* Curaciona, no acumules: cuando arrancas una tarea, decide conscientemente qué necesita el agente leer. Resiste la tentación de "por si acaso".

* Usa sub-agents para subtareas costosas en contexto (exploración de codebase, búsqueda de bugs, investigación de stack traces). La conversación verbosa NO debe vivir en el thread principal.

* Compacta o reinicia cada 15-20 turnos en una sesión agentic. La pérdida de coherencia es empíricamente notable a partir de ahí.

* Trata el archivo de contexto persistente como código: revisión por PR, versionado, tests (si tu equipo es serio, add un linter de AGENTS.md).
