# 📄 Pilar 3 — El Prompt + Integración 🔴 — 21 min
...
Anthropic lo dice explícitamente en su guía oficial 2026:

> "el mejor prompt no es el más largo ni el más complejo. Es el que logra tu objetivo de forma fiable con la mínima estructura necesaria."

## Qué del prompt engineering clásico sigue vigente

| Técnica                                   | Vigente en 2026 con razonadores | Comentario                                                                      |
| ----------------------------------------- | ------------------------------- | ------------------------------------------------------------------------------- |
| Chain-of-Thought ("think step by step")   | NO o contraproducente           | El modelo ya razona; tu instrucción consume thinking budget útil                |
| Few-shot (ejemplos)                       | Útil pero menos crítico         | Razonadores infieren mejor desde 0-shot. Probar 0-shot antes de añadir ejemplos |
| Role/persona prompting                    | Casi neutral                    | "You are an expert" no mejora coding según research; reserva role para creativo |
| Estructura XML / delimitadores            | Sí                              | Anthropic lo recomienda explícitamente para inputs largos                       |
| Prefilling (assistant primer character)   | Sí                              | Útil para format compliance estricto (JSON schemas)                             |
| Framing positivo ("usa X" vs "no uses Y") | Sí                              | Empíricamente medible; reglas en positivo funcionan mejor                       |
| Megaprompts >2k tokens                    | NO                              | Pierden foco, consumen thinking budget. Más prompt ≠ mejor prompt               |

## La anatomía de un prompt técnico 2026

Convergencia de Anthropic best-practices, OpenAI Cookbook y Codex AGENTS.md guidance:
...
* Vaguedad — "hazme un dashboard". Output genérico, mucho ida y vuelta. Cuesta más turnos que escribir bien el prompt una vez.

* Sobre-especificación micro — listar instrucciones step-by-step para algo trivial. Los razonadores pierden recall cuando los limitas en exceso (Anthropic notó esto explícitamente en Opus 4.7: instrucciones tipo "be conservative" reducen recall reportado pese a la misma profundidad).

* Megaprompt gigantesco — meter convenciones, código, ejemplos, restricciones, todo en cada turn. Confunde, consume contexto. Anti-principio Anthropic: "the best prompt isn't the longest or most complex".

* No proporcionar criterios de éxito — el modelo entrega lo que cree que tú quieres, no lo que pasa los tests. Resultado: PRs que "parecen bien" pero fallan en CI.

* Mezclar varias tareas en un solo turno — divide en sub-prompts encadenados. Si necesitas hacer A, luego B basado en el resultado de A, no metas todo junto.

* Re-pegar inline el AGENTS.md/CLAUDE.md en cada prompt — ya está cargado por la herramienta. Solo añade ruido.
...
* Mapa primero: pedir al agente que identifique símbolos, dependencias y riesgos, sin tocar código.

* Aprobación del mapa por el humano.

* Ejecución en pasos reversibles con commits intermedios.
...
**arXiv 2506.04210 — "Does Thinking More always Help?"**

Extender thinking con "wait, rethink" funciona, pero multiple independent reasoning paths + majority vote supera 20% al thinking más largo. Implicación: paralelizar (sub-agents independientes) supera a "pensar más fuerte".

**arXiv 2510.21413 — Context Engineering for AI Agents in Open-Source Software (oct 2025)**

Análisis empírico que confirma que las decisiones de contexto y harness explican mucha más varianza que el prompt en sí.

**OpenAI Cookbook 2026**

Guía explícita de NO añadir CoT a GPT-5.x; usar effort levels (low/medium/high) como dial principal.

## Tu kit de prompting

Si tuvieras que llevarte cinco prácticas operativas de esta sección:

* Empieza por el outcome y los criterios de éxito, no por los steps. Deja que el modelo decida cómo.

* Restricciones explícitas ("no introduzcas dependencias nuevas", "mantén el patrón X"). Las restricciones reducen el espacio de soluciones malas.

* Pide clarificación si hay ambigüedad: añade "si tienes dudas sobre X, pregunta antes de implementar". Te ahorra rework.

* Para razonadores: prompt corto y directo. Si te encuentras escribiendo un megaprompt, párate y rebaja.

* Probar 0-shot antes de few-shot. La mitad de las veces los ejemplos sobran.
...
* **Pilar 2:** AGENTS.md + services/users.py como referencia de patrón. Sin sub-agent (tarea contenida).
...
* **Pilar 2:** el diff + AGENTS.md con convenciones del proyecto. NO meter todo el repo.
...
* Megaprompt + todo el repo en contexto + modelo top: confundir "más es mejor" con efectividad. Resultado: tokens caros, calidad baja, context rot inmediato.

* Herramienta agentic potente + sin AGENTS.md + prompt vago: el agente improvisa convenciones que no son las tuyas. Genera código inconsistente con el resto del proyecto.

* Cambiar de herramienta cada semana mientras el contexto y los prompts no mejoran: la varianza de resultados está en pilares 2 y 3, no en pilar 1.

* Test-first prompt con cloud autonomous sin haber probado primero local: te encuentras 30 minutos después con un PR que falla CI por algo que habrías visto en el primer turno.
...
* Si tu output con IA es inconsistente con las convenciones de tu proyecto → tu cuello de botella está en pilar 2 (contexto). Mejora AGENTS.md.
...
**AGENTS.md:** archivo de contexto persistente, estándar de facto desde 2025–2026, soportado por la mayoría de herramientas.
...
* El framework de los 3 pilares interiorizado — herramienta, contexto, prompt — como capas co-iguales, no jerárquicas.

* El árbol de decisión que viste en la parte 2 de esta lección: caracterizar tarea → elegir herramienta → preparar contexto → escribir prompt → ejecutar.

* La regla de los umbrales de contexto: 50% empieza degradación, 70% compactar, 90% reset.

* La idea de que "modelo no es el cuello de botella" — el harness, el contexto y el prompt explican más varianza que la versión del modelo.
