# 📄 Anatomía de un backlog AI-ready 🔴— 16 min
...
* El nivel correcto del prompt al copiloto es la story, no el epic. Pedirle a un agente que implemente un epic completo es la forma más rápida de quemar tokens y obtener algo aproximado. La granularidad de story (1-2 días humano-equivalente) es donde los copilotos rinden bien.

* Las tasks son artefactos del agente más que del humano. Si la story está bien definida, las tasks las puede generar el copiloto en su EPE (Explore-Plan-Execute, S3) sin que tú tengas que pre-mascarlas.

* Los AC son la única defensa contra la "false completeness" de la IA. Si no los pones, el agente va a producir algo que parece terminado pero le falta el caso que tú nunca mencionaste.
...
|             | Original                                          | Lectura AI-aware                                                                                       |
| ----------- | ------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| Independent | Se puede entregar sin dependencias bloqueantes    | El agente puede trabajar en ella sin que falten specs/contextos de otra story que aún no existe        |
| Negotiable  | El qué está abierto a discusión                   | No es un encargo cerrado escrito en piedra; el equipo puede ajustarlo en refinamiento                  |
| Valuable    | Aporta valor concreto a un usuario o al sistema   | Si no puedes describir el valor, el copiloto va a inventar uno y mal                                   |
| Estimable   | El equipo puede estimarla con confianza razonable | Tienes contexto suficiente (specs, repo, similares previas) para que humanos + IA estimen sin adivinar |
| Small       | Cabe en un sprint, idealmente más pequeña         | Cabe en una sesión de copiloto sin que el contexto se rompa (regla 50/70/90 de S1)                     |
| Testable    | Sabes cómo verificar que está hecha               | Hay AC observables, automatizables idealmente con Gherkin o asserts                                    |
...
📺 https://www.youtube.com/watch?v=Cb5R2JbnA-c
...
|                       | Backlog (product layer)        | OpenSpec (spec layer)                        |
| --------------------- | ------------------------------ | -------------------------------------------- |
| Pregunta que responde | ¿Qué se entrega y por qué?     | ¿Cómo se construye exactamente?              |
| Audiencia primaria    | Producto, stakeholders, equipo | Equipo + agente que va a implementar         |
| Granularidad          | User stories (días)            | Cambios concretos al código (commits)        |
| Persistencia          | Linear/Jira/Notion (live)      | openspec/ en el repo (versionado con código) |
| Quién lo escribe      | PO + equipo                    | Tech lead + agente, sobre la story           |
