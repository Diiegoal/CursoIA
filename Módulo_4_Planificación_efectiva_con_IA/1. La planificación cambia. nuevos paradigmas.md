# 📄 La planificación cambia: nuevos paradigmas 🔴— 11 min
...
| Task planning (tradicional)            | Capability planning (IA-aware)                                                                                                   |
| -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| "Esto son 5 SP, vamos al sprint"       | "Esto son 5 SP si el contexto del repo está fresco en CLAUDE.md, si la spec OpenSpec existe y si hay tests del módulo adyacente" |
| Velocity = puntos completados / sprint | Velocity = outcomes entregados / sprint, segmentados por tipo de tarea (greenfield vs legacy vs investigación)                   |
| Una sola estimación                    | Tres estimaciones: con copiloto + contexto bueno / con copiloto + contexto pobre / sin copiloto                                  |
| Dependencias = otros tickets           | Dependencias = otros tickets + estado del contexto (specs, tests, docs) + disponibilidad del MCP correcto                        |
