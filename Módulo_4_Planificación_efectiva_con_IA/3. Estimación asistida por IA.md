# 📄 Estimación asistida por IA 🔴— 13 min
...
| Técnica                     | Para qué sirve bien                                                                              | Para qué falla con IA                                                                                                                                 |
| --------------------------- | ------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| Story Points (Fibonacci)    | Comparar complejidad relativa entre ítems conocidos por el equipo. Predecir capacidad de sprint. | No tiene sentido entre equipos. No es para reportar a stakeholders.                                                                                   |
| T-shirt sizes (XS-S-M-L-XL) | Estimación rápida en upstream (epics, descubrimiento). Antes del refinamiento.                   | Demasiado grueso para sprint planning operativo.                                                                                                      |
| Horas/días                  | Compromisos contractuales con cliente externo. Facturación.                                      | Anti-patrón en sprint planning interno: invita a Parkinson, mide individuos, falla cuando hay copiloto porque las horas individuales pierden sentido. |
...
📊 Lo que sí sabemos (síntesis de Index.dev, Faros AI 2026, Stack Overflow 2025, DORA 2025):
...
| Tipo de trabajo                 | Aceleración real con IA | Recomendación al estimar              |
| ------------------------------- | ----------------------- | ------------------------------------- |
| Implementación greenfield       | 20-30%                  | Aplicar multiplier 0.85               |
| Implementación sobre legacy     | 5-10%                   | No ajustar                            |
| Refactor (con tests existentes) | 30-40%                  | Aplicar multiplier 0.7                |
| Refactor (sin tests)            | -20% (más lento)        | Aumentar 1.3×                         |
| Testing (escribir tests nuevos) | 30-40%                  | Aplicar 0.7                           |
| Debugging real                  | 10-20%                  | Aplicar 0.85 si el bug es clásico     |
| Investigación / spike           | 0-10%                   | No ajustar                            |
| Documentación                   | 50%+                    | Aplicar 0.5                           |
| Code review                     | 0-10%                   | No ajustar (IA aún no review-ea bien) |
| Reuniones de refinamiento       | 10-20%                  | Reducir tiempo de meeting, no de prep |
...
* Verificación tax: cada PR generado por agente requiere review humano más cuidadoso. El tiempo de review subió.

* Quality tax: PR sizes +150% con IA, bug counts +9% (Faros 2026). Más bugs significan más interrupciones de soporte mid-sprint.

* Tooling churn: las herramientas (Cursor, Claude Code, Linear Agent) se actualizan cada 2-4 semanas. Hay tiempo de adaptación constante.

* Inestabilidad de modelos: los modelos cambian de día. Un patrón que funcionaba con Claude Sonnet 4.5 puede fallar con 4.6. Se invierte tiempo recalibrando.
